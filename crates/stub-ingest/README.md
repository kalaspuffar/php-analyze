...
| `--capture-dir <path>` | *(none)* | Write each successfully-validated POST body to `<path>/batch-NNNN.msgpack` (4-digit zero-padded monotonic counter starting at `0001`). The captured bytes are byte-identical to what the client sent (no `rmp_serde` round-trip). The directory MUST already exist and be writable — the stub validates this at startup and exits non-zero with a single stderr line if not. Composes with `--respond-with` and `--simulate-slow` (capture happens BEFORE both, so even configured-failure responses still capture). A per-POST write failure surfaces as a `500 capture write failed` response, and the in-memory store is NOT updated for that POST (preserving the "successful capture = successful store" invariant). Used by `tools/capture-fixtures.sh` to populate `tools/captured-batches/<workload>/` with real wire-format reference bytes for downstream parser tests. |
